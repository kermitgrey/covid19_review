...
def generateDeathsData():
    '''
    The CDC (Center for Disease Control) compiles national death
    statistics from all 50 states on an annual basis.
    This fcn generates one dataframe containing all available
    data for files existing in the datadir directory and writes
    it out to file.
    '''
    
    datafiles = list()
    datadir = 'covid19/'  # location of original datafiles
    dfdeaths = DataFrame()
    
    # generate a list of all data files existing in datadir
    for file in os.listdir(datadir):
        if file.startswith('Deaths') & file.endswith('.txt'):
            datafiles.append(file)
    
    # loop through file list and read the data into temp df
    # then concatenate each successive file to create one df
    # containing all individual file data.
    for fname in sorted(datafiles):
        year = fname[6:10]
        dftemp = pd.read_table(datadir+fname,
                              usecols=[2,4,5,6,7,8,9,10,11],
                              dtype={'State Code': str,
                                     'County Code': str},
                              skipfooter=75,
                              engine='python')
        dftemp['reportyear'] = year           # add the reporting year to df
        dfdeaths = dfdeaths.append(dftemp, ignore_index=True)  # append to df
    
    # write out data to file
    dfdeaths.to_csv(datadir+'dataOut/cdc_deathsout.csv', index=False, sep='|')   
...
def generateCovidDaily(df, fileLabel='cases'):
    ...
    dfdaily.to_csv(datadir+'dataOut/'+filename, index=False)
